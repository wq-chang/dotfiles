...
| `config/pi/`                            | Pi agent configuration, symlinked to `~/.pi/agent` via `modules/pi.nix` (mkOutOfStoreSymlink — edits are live immediately). Contains `skills/` (incl. `harness-spec`, `harness-gauntlet`), `harness/` (prompts, guard script, CLI adapters), `extensions/` (incl. `harness-guard.ts`).                                                                                                                                                        |
| `config/pi/harness/`                    | Agent harness: portable markdown workflow (skills + prompts) implementing spec-review and constraint-gauntlet phases, `sensors.md` (sensor declaration format for AGENTS.md/harness.yaml), and `guard/harness-guard.sh` (end-of-session sensor enforcement shared by pi extension and Claude Stop hook; `--stamp` mode fingerprints checkpoint logs, `--abandon` disarms a run). See `.pi-memories/specs/2026-07-26-agent-harness-design.md`. |
